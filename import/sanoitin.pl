...
sub infList {
  my ($text) = @_;
  my $resp = $Ua->post('http://127.0.0.1:8100/perusta', {
    format => 'json',
    text => $text
  });
  ...
  return { %{$wordCounts->{ok}}, %{$wordCounts->{notFound}} };
...
#print "disable keys...\n";
#$Conn->do("ALTER TABLE word_occurrence DISABLE KEYS");
my %wordIds;
...
#$Conn->do("ALTER TABLE word_occurrence ENABLE KEYS");
